...
# ========= НАСТРОЙКИ ==========
TOKEN = "8708654790:AAEG-HQcgYgLykvceLpGJUiQFLOuS3d8c2k"  # ← НОВЫЙ РАБОЧИЙ ТОКЕН
CHANNEL = "@Netizenshop"
FILE = "products.xlsx"

# Обычные эмодзи (как запасной вариант)
DEFAULT_EMOJI = {
    'update': '📌',
    'check': '✅',
    'battery': '🔋',
    ...
    for model in model_order:
        ...
        for (_, storage, sim_type), items in sorted(model_items.items(), key=lambda x: int(x[0][1].replace('GB', ''))):
            ...
            for color, price, emoji_id in sorted(items, key=lambda x: x[0]):
                color_emoji = get_emoji_html(emoji_id, DEFAULT_EMOJI['dot'])
